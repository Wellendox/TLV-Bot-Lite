...
internal class DoUserCheck : PreconditionAttribute
{
    ...
    public override Task<PreconditionResult> CheckRequirementsAsync(IInteractionContext context, ICommandInfo commandInfo, IServiceProvider services)
    {
        ...
        {
            // The approach here entirely depends on how you construct your custom ID. In this case,
            // the format is: unique-name:*,*

            // here the name and wildcards are split by ':'
            var param = componentContext.Data.CustomId.Split(':');

            // here we determine that we should always check for the first ',' present. This will
            // deal with additional wildcards by always selecting the first wildcard present.
